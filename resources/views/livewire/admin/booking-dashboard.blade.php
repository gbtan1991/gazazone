<div x-data="{ panel: @entangle('showPanel') }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-stone-900">Buchungen</h1>
            <p class="text-sm text-stone-500 mt-0.5">Alle Kundentermine verwalten</p>
        </div>
        <button
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        @foreach ([
            ['Ausstehend',      $this->stats['pending'],   'text-amber-700',  'bg-amber-50',  'border-amber-200'],
            ['Bestätigt',       $this->stats['approved'],  'text-green-700',  'bg-green-50',  'border-green-200'],
            ['Storniert',       $this->stats['cancelled'], 'text-red-700',    'bg-red-50',    'border-red-200'],
            ['Heute',           $this->stats['today'],     'text-stone-700',  'bg-stone-50',  'border-stone-200'],
    ...
    <div class="bg-white border border-stone-200 rounded-2xl p-4 mb-5 flex flex-wrap gap-3 items-center">
        ...
        <div class="flex gap-1.5 flex-wrap">
            @foreach (['all' => 'Alle', 'pending' => 'Ausstehend', 'approved' => 'Bestätigt', 'cancelled' => 'Storniert'] as $val => $lbl)
            <button
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-stone-100 bg-stone-50/60">
                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider">Kunde</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider hidden sm:table-cell">Datum & Uhrzeit</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider hidden lg:table-cell">Leistung</th>
                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-stone-500 uppercase tracking-wider hidden md:table-cell">Telefon</th>
                ...
                <tbody class="divide-y divide-stone-100">
                    ...
                    <tr class="hover:bg-stone-50/50 transition-colors group">
                        ...
                        <td class="px-5 py-4">
                            <span @class([
                                ...
                                @php $statusMap = ['pending' => 'Ausstehend', 'approved' => 'Bestätigt', 'cancelled' => 'Storniert']; @endphp
                                {{ $statusMap[$booking->status] ?? ucfirst($booking->status) }}
                            </span>
                        </td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
                                ...
                                <div class="relative">
                                    ...
                                    <div x-show="open" @click.outside="open = false"
                                         ...
                                         class="absolute right-0 mt-1 w-40 bg-white border border-stone-200 rounded-xl shadow-lg z-20 py-1 text-xs">
                                        ...
                                        <button
                                            ...
                                            wire:confirm="Diese Buchung dauerhaft löschen?"
                                            @click="open=false"
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-16 text-stone-400">
                            ...
                            <p class="text-sm font-semibold">Keine Buchungen gefunden</p>
                            <p class="text-xs mt-1 text-stone-300">Filter anpassen oder manuell eine Buchung erstellen.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    >
        <div class="flex items-center justify-between px-6 py-5 border-b border-stone-100 shrink-0">
            <div>
                ...
                <p class="text-xs text-stone-400 mt-0.5">
                    {{ $editingId ? 'Buchungsdetails unten aktualisieren.' : 'Buchung manuell für einen Kunden erstellen.' }}
                </p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            ...
            <fieldset class="space-y-4">
                <legend class="text-xs font-bold uppercase tracking-widest text-stone-400 mb-3">Kundendaten</legend>

                <div>
                    <label class="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wide">Vollständiger Name</label>
                    <input type="text" wire:model="form_name" placeholder="Hans Muster"
                ...
                <div>
                    ...
                    <input type="email" wire:model="form_email" placeholder="hans@beispiel.ch"
                           class="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-900 transition placeholder-stone-300">
                    @error('form_email') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wide">Telefon</label>
            ...
            <fieldset class="space-y-4">
                <legend class="text-xs font-bold uppercase tracking-widest text-stone-400 mb-3">Termindetails</legend>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wide">Datum</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wide">Uhrzeit</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wide">Leistung</label>
                ...
                <div>
                    ...
                    <textarea wire:model="form_notes" rows="3"
                              placeholder="Interne Notizen oder Kundenwünsche…"
        ...
        <div class="shrink-0 px-6 py-4 border-t border-stone-100 flex justify-between items-center bg-stone-50/50">
            ...
            >
                <span wire:loading.remove wire:target="save">
                    {{ $editingId ? 'Änderungen speichern' : 'Buchung erstellen' }}
                </span>
