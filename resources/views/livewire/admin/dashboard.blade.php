<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-xl font-bold text-stone-900">Buchungen</h1>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-2xl border border-stone-200 p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-2">Aktuell</p>
            <p class="text-3xl font-bold text-stone-900">{{ $this->counts['current'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-stone-200 p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-2">Heute</p>
            <p class="text-3xl font-bold text-blue-600">{{ $this->counts['today'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-stone-200 p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-2">Abgeschlossen</p>
            <p class="text-3xl font-bold text-green-600">{{ $this->counts['completed'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-stone-200 p-5">
            <p class="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-2">Storniert</p>
    ...
    <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 px-5 pt-4 pb-0 border-b border-stone-100">
            <div class="flex gap-1">
                @foreach (['current' => 'Aktuell', 'completed' => 'Abgeschlossen', 'cancelled' => 'Storniert'] as $key => $label)
                <button
                    type="button"
            ...
            <div class="sm:ml-auto flex flex-col sm:flex-row gap-2 pb-3">
                <input
                    ...
                    placeholder="Name, E-Mail, Leistung suchen…"
                ...
                <button type="button" wire:click="$set('dateFilter', '')"
                        class="text-xs font-semibold text-stone-400 hover:text-stone-700 px-2">Datum löschen</button>
                @endif
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-xs font-bold text-stone-400 uppercase tracking-wider bg-stone-50 border-b border-stone-100">
                        ...
                        <th class="px-5 py-3 hidden md:table-cell">Leistung</th>
                        <th class="px-5 py-3">Datum & Uhrzeit</th>
                ...
                <tbody class="divide-y divide-stone-50">
                            ...
                            <div class="flex items-center justify-end gap-1.5">
                                @if ($booking->status === 'current')
                                    <button
                                        ...
                                        wire:confirm="Diese Buchung als abgeschlossen markieren?"
                                    ...
                                    <button
                                        ...
                                        wire:confirm="Diese Buchung stornieren?"
                                ...
                                @elseif ($booking->status === 'cancelled' || $booking->status === 'completed')
                                    <button
                                        ...
                                        wire:confirm="Buchung wieder auf aktuell setzen?"
                    ...
                    <tr>
                        <td colspan="5" class="px-5 py-16 text-center">
                            ...
                            <p class="text-sm font-semibold text-stone-400">Keine Buchungen gefunden</p>
                            <p class="text-xs text-stone-300 mt-1">Suche oder Datumsfilter anpassen.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
